...
const register = (session, body) => {
  // Creates User from body data
  const bio = body.bio ? body.bio : "";
  const query = `CREATE (user:User {name: '${body.name}', username: '${
    body.username
  }', wallet_address: '${
    body.wallet_address
  }', bio: '${bio}', nonce: '${generateNonce()}'});`;
  return session
    .run(query)
    .then((results) => {
      return {
...
const getNonce = (session, wallet_address) => {
  // Login User & get JWT authentication
...
const login = (session, wallet_address, signed_nonce, setTokenInCookie) => {
  // Login User & get JWT authentication
  return getByWalletAddress(session, wallet_address)
    .then((response) => {
      if (response.success) {
...
const updateUser = function (session, wallet, filter, newUser) {
  ...
  const filteredUser = Object.fromEntries(
    Object.entries(newUser).filter(([key, value]) => filter.includes(key))
  );

  // Check for existence of required fields.
  for (let filterName of filter) {
    if (!(filterName in newUser))
...
const getFollowing = (session, wallet_address) => {
  const query = `MATCH ((a:User{wallet_address:"${wallet_address}"})-[r:FOLLOWING]->(b:User)) RETURN b`;
  return session
    .run(query)
    .then((results) => {
